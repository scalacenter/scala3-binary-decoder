...
class BinaryDecoderStatsFull extends BinaryDecoderSuite:
  ...
  def decodeAllFromCsvFile(fileName: String)(using ThrowOrWarn): Unit =
    ...
    for line <- csv.getLines.drop(1) do
      ...
      try
        val (classCounter, methodCounter, _, _) = tryDecodeAll(org, artifact, version)
  ...
  def tryDecodeAll(org: String, artifact: String, version: String)(using
      ThrowOrWarn
    ...
    def tryWith(keepOptional: Boolean, keepProvided: Boolean): Option[(Counter, Counter, Counter, Counter)] =
      try
